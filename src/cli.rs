...
fn validate_config() -> Result<(), AppError> {
    ...
    println!("AetherGateway configuration");
    ...
    println!("AetherGateway configuration valid: {warnings} warning(s).");
    Ok(())
}
...
fn export_backup(path: &str) -> Result<(), AppError> {
    ...
    println!("AetherGateway backup written to {path}");
    Ok(())
}
...
fn validate_backup(path: &str) -> Result<(), AppError> {
    ...
    let api_key_count = backup
        ...
        .unwrap_or(0);
    println!(
        "AetherGateway backup valid: {user_count} user(s), {api_key_count} API key record(s), contains_secrets={}",
...
fn restore_backup(path: &str) -> Result<(), AppError> {
    ...
    println!(
        "AetherGateway backup restored to {} and {}",
        auth_store.location(),
...
fn load_backup(path: &str) -> Result<LocalBackupFile, AppError> {
    ...
    if backup.schema_version != 1 || backup.service != "model-port" {
        return Err(AppError::InvalidRequest(
            "not a supported AetherGateway backup".to_owned(),
        ));
    }
