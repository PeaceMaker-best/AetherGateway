...
pub(crate) fn validate_environment() -> Result<(), AppError> {
    ...
    if database::enterprise_mode_enabled()? {
        validate_enterprise_security(EnterpriseSecurityValues {
            cookie_secure: std::env::var("AETHERGATEWAY_ADMIN_COOKIE_SECURE").ok(),
            require_control_api_keys: std::env::var("AETHERGATEWAY_REQUIRE_CONTROL_API_KEYS").ok(),
            disable_csrf: std::env::var("AETHERGATEWAY_DISABLE_CSRF").ok(),
            allowed_origins: std::env::var("AETHERGATEWAY_ALLOWED_ORIGINS").ok(),
            trusted_proxies: std::env::var("AETHERGATEWAY_TRUSTED_PROXIES").ok(),
        })?;
    }
    Ok(())
}
...
fn validate_enterprise_security(values: EnterpriseSecurityValues) -> Result<(), AppError> {
    if !flag_enabled(values.cookie_secure.as_deref()) {
        return Err(AppError::Config(
            "AETHERGATEWAY_ENTERPRISE_MODE requires AETHERGATEWAY_ADMIN_COOKIE_SECURE=1".to_owned(),
        ));
    }
    if !flag_enabled(values.require_control_api_keys.as_deref()) {
        return Err(AppError::Config(
            "AETHERGATEWAY_ENTERPRISE_MODE requires AETHERGATEWAY_REQUIRE_CONTROL_API_KEYS=1".to_owned(),
        ));
    }
    if flag_enabled(values.disable_csrf.as_deref()) {
        return Err(AppError::Config(
            "AETHERGATEWAY_DISABLE_CSRF is forbidden when AETHERGATEWAY_ENTERPRISE_MODE=1".to_owned(),
        ));
    }
    let allowed_origins = values
        ...
        .ok_or_else(|| {
            AppError::Config(
                "AETHERGATEWAY_ENTERPRISE_MODE requires explicit AETHERGATEWAY_ALLOWED_ORIGINS".to_owned(),
            )
        })?;
    if allowed_origins
        .split(',')
    ...
    {
        return Err(AppError::Config(
            "AETHERGATEWAY_ENTERPRISE_MODE requires HTTPS-only AETHERGATEWAY_ALLOWED_ORIGINS".to_owned(),
        ));
    }
    if values
        .trusted_proxies
    ...
    {
        return Err(AppError::Config(
            "AETHERGATEWAY_ENTERPRISE_MODE requires explicit AETHERGATEWAY_TRUSTED_PROXIES".to_owned(),
        ));
    }
    Ok(())
}
...
fn validate_finalization_drain_timeout() -> Result<(), AppError> {
    let Some(value) = std::env::var("AETHERGATEWAY_FINALIZATION_DRAIN_TIMEOUT_SECONDS")
        .ok()
    ...
    let seconds = value.parse::<u64>().map_err(|_| {
        AppError::Config(
            "AETHERGATEWAY_FINALIZATION_DRAIN_TIMEOUT_SECONDS must be an integer".to_owned(),
        )
    })?;
    if !(1..=300).contains(&seconds) {
        return Err(AppError::Config(
            "AETHERGATEWAY_FINALIZATION_DRAIN_TIMEOUT_SECONDS must be between 1 and 300".to_owned(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn secure_values() -> EnterpriseSecurityValues {
        EnterpriseSecurityValues {
            ...
            disable_csrf: None,
            allowed_origins: Some("https://aethergateway.example.com".to_owned()),
    ...
    fn enterprise_security_requires_fail_closed_browser_and_client_auth() {
        ...
        let mut values = secure_values();
        values.allowed_origins = Some("http://aethergateway.example.com".to_owned());
        assert!(validate_enterprise_security(values).is_err());
