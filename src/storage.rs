...
const STATE_TABLE: &str = "aethergateway_state";
...
impl JsonStore {
    pub fn open(namespace: &str) -> Result<Self, AppError> {
        let database_url = database_url().ok_or_else(|| {
            AppError::Config(
                "AETHERGATEWAY_DATABASE_URL is required; current releases store auth and control state in PostgreSQL"
                    .to_owned(),
            )
        })?;
...
fn spawn_postgres_worker(
    ...
    let thread_name = format!("aethergateway-postgres-{namespace}");
...
mod tests {
    ...
    fn temporary_test_directory(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "aethergateway-storage-{label}-{}-{sequence}",
            std::process::id()
        ))
    }
