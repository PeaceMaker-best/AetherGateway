...
async fn count_tokens_inner(
    ...
    let classification = request_routing_header(
        headers,
        &DATA_CLASSIFICATION,
        "x-aethergateway-data-classification",
        32,
    )?
    .map(|value| {
        DataClassification::parse(&value).ok_or_else(|| {
            AppError::InvalidRequest(
                "x-aethergateway-data-classification must be unknown, sensitive, internal, or public"
                    .to_owned(),
            )
        })
    })
    ...
    let requested_mode = request_routing_header(
        headers,
        &HYBRID_MODE,
        "x-aethergateway-hybrid-mode",
        32,
    )?
    .map(|value| {
        HybridMode::parse(&value).ok_or_else(|| {
            AppError::InvalidRequest(
                "x-aethergateway-hybrid-mode must be local_strict, local_first, balanced, or cloud_first"
                    .to_owned(),
            )
        })
    })
...
async fn handle_inference(
    ...
    let routing_profile = match request_routing_header(
        &headers,
        &ROUTING_PROFILE,
        "x-aethergateway-routing-profile",
        32,
    ) {
        Ok(value) => value,
        Err(err) => {
        ...
        Some(value) => match RoutingProfile::parse(value) {
            ...
            None => {
                return Err(record_inference_rejection(
                    ...
                    "validation",
                    AppError::InvalidRequest(
                        "x-aethergateway-routing-profile must be quality, balanced, economy, or latency"
                            .to_owned(),
                    ),
                    started,
                ));
...
fn request_tenant_scope(
            ...
            Some(Ok(value)) => {
                ...
                if !crate::domain::valid_tenant_identifier(value) {
                    return Err(AppError::InvalidRequest(
                        "AetherGateway tenant scope headers contain an invalid identifier".to_owned(),
                    ));
                }
                Ok(Some(value))
            }
            Some(Err(_)) => Err(AppError::InvalidRequest(
                "AetherGateway tenant scope headers must be ASCII".to_owned(),
            )),
        })
    ...
    match requested.as_slice() {
        ...
        [Some(_), Some(_), Some(_)] => Err(AppError::Forbidden(
            "requested AetherGateway tenant scope is not bound to this API key".to_owned(),
        )),
        _ => Err(AppError::InvalidRequest(
            "x-aethergateway-organization-id, x-aethergateway-project-id, and x-aethergateway-environment-id must be supplied together"
                .to_owned(),
        )),
    }
}
...
fn request_traffic_class(headers: &HeaderMap) -> Result<String, AppError> {
    ...
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(AppError::InvalidRequest(
            "x-aethergateway-traffic-class must be business, batch, synthetic, or diagnostic"
                .to_owned(),
        )),
    }
}
...
fn validate_message_request(request: &AnthropicRequest) -> Result<(), AppError> {
    ...
    let max_output_tokens = env_u64("AETHERGATEWAY_MAX_OUTPUT_TOKENS", 131_072);
...
fn validate_anthropic_input(request: &AnthropicRequest) -> Result<(), AppError> {
    let max_model_name_chars = env_usize("AETHERGATEWAY_MAX_MODEL_NAME_CHARS", 240);
    let max_messages = env_usize("AETHERGATEWAY_MAX_MESSAGES", 200);
    let max_messages_json_chars = env_usize("AETHERGATEWAY_MAX_MESSAGES_JSON_CHARS", 2 * 1024 * 1024);
    let max_system_json_chars = env_usize("AETHERGATEWAY_MAX_SYSTEM_JSON_CHARS", 256 * 1024);
    let max_tools = env_usize("AETHERGATEWAY_MAX_TOOLS", 256);
    let max_tools_json_chars = env_usize("AETHERGATEWAY_MAX_TOOLS_JSON_CHARS", 1024 * 1024);
...
mod tests {
    ...
    fn tenant_scope_headers_are_assertions_not_authority() {
        let bound =
            crate::domain::TenantScope::from_strings("org_dave", "prj_signalfoundry", "env_test");
        let mut matching = HeaderMap::new();
        matching.insert(&ORGANIZATION_ID, HeaderValue::from_static("org_dave"));
        matching.insert(&PROJECT_ID, HeaderValue::from_static("prj_signalfoundry"));
        ...
        invalid.insert(&PROJECT_ID, HeaderValue::from_static("prj_signalfoundry"));
