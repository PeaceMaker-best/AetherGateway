...
pub(crate) fn database_url() -> Option<String> {
    non_empty_env("AETHERGATEWAY_DATABASE_URL")
}

pub(crate) fn enterprise_database_url() -> Option<String> {
    non_empty_env("AETHERGATEWAY_ENTERPRISE_DATABASE_URL").or_else(database_url)
}

pub(crate) fn enterprise_mode_enabled() -> Result<bool, AppError> {
    parse_flag(
        "AETHERGATEWAY_ENTERPRISE_MODE",
        non_empty_env("AETHERGATEWAY_ENTERPRISE_MODE").as_deref(),
    )
}

pub(crate) fn validate_configuration() -> Result<(), AppError> {
    let enterprise_mode = non_empty_env("AETHERGATEWAY_ENTERPRISE_MODE");
    let control_url = database_url();
    let ledger_url = non_empty_env("AETHERGATEWAY_ENTERPRISE_DATABASE_URL");
    let tls_mode = non_empty_env("AETHERGATEWAY_DATABASE_TLS_MODE");
    let max_connections = non_empty_env("AETHERGATEWAY_DATABASE_MAX_CONNECTIONS");
    let min_connections = non_empty_env("AETHERGATEWAY_DATABASE_MIN_CONNECTIONS");
    let acquire_timeout = non_empty_env("AETHERGATEWAY_DATABASE_ACQUIRE_TIMEOUT_SECS");
...
fn validate_values(values: DatabaseValidationValues<'_>) -> Result<(), AppError> {
    let enterprise = parse_flag("AETHERGATEWAY_ENTERPRISE_MODE", values.enterprise_mode)?;

    if values.control_url.is_none() {
        return Err(AppError::Config(
            "AETHERGATEWAY_DATABASE_URL is required; current releases use PostgreSQL for all runtime state"
                .to_owned(),
        ));
    }
    if let Some(url) = values.control_url {
        validate_database_url("AETHERGATEWAY_DATABASE_URL", url)?;
    }
    if let Some(url) = values.ledger_url {
        validate_database_url("AETHERGATEWAY_ENTERPRISE_DATABASE_URL", url)?;
    }

    ssl_mode_for(values.tls_mode, enterprise)?;

    let max_connections =
        parse_optional_u32("AETHERGATEWAY_DATABASE_MAX_CONNECTIONS", values.max_connections)?
            .unwrap_or(DEFAULT_POOL_SIZE);
    if max_connections == 0 {
        return Err(AppError::Config(
            "AETHERGATEWAY_DATABASE_MAX_CONNECTIONS must be at least 1".to_owned(),
        ));
    }
    let min_connections =
        parse_optional_u32("AETHERGATEWAY_DATABASE_MIN_CONNECTIONS", values.min_connections)?
            .unwrap_or(0);
    if min_connections > max_connections {
        return Err(AppError::Config(
            "AETHERGATEWAY_DATABASE_MIN_CONNECTIONS must not exceed AETHERGATEWAY_DATABASE_MAX_CONNECTIONS"
                .to_owned(),
        ));
    }
    let acquire_timeout = parse_optional_u64(
        "AETHERGATEWAY_DATABASE_ACQUIRE_TIMEOUT_SECS",
        values.acquire_timeout,
    )?
    .unwrap_or(DEFAULT_ACQUIRE_TIMEOUT_SECS);
    if acquire_timeout == 0 {
        return Err(AppError::Config(
            "AETHERGATEWAY_DATABASE_ACQUIRE_TIMEOUT_SECS must be at least 1".to_owned(),
        ));
    }
    Ok(())
}
...
pub(crate) async fn connect_pool(
    ...
    let max_connections = max_connections
        .or_else(|| env_u32("AETHERGATEWAY_DATABASE_MAX_CONNECTIONS"))
        .unwrap_or(DEFAULT_POOL_SIZE)
        .max(1);
    let min_connections = env_u32("AETHERGATEWAY_DATABASE_MIN_CONNECTIONS")
        .unwrap_or(0)
        .min(max_connections);
    let acquire_timeout = Duration::from_secs(
        env_u64("AETHERGATEWAY_DATABASE_ACQUIRE_TIMEOUT_SECS")
            .unwrap_or(DEFAULT_ACQUIRE_TIMEOUT_SECS)
            .max(1),
    );
...
fn connect_options(database_url: &str, enterprise: bool) -> Result<PgConnectOptions, AppError> {
    ...
    PgConnectOptions::from_str(database_url)
        .map(|options| options.ssl_mode(ssl_mode).application_name("aethergateway"))
        .map_err(|_| {
            AppError::Config(
...
fn configured_ssl_mode(enterprise: bool) -> Result<PgSslMode, AppError> {
    let configured = non_empty_env("AETHERGATEWAY_DATABASE_TLS_MODE");
...
fn ssl_mode_for(configured: Option<&str>, enterprise: bool) -> Result<PgSslMode, AppError> {
    ...
    {
        return Err(AppError::Config(
            "AETHERGATEWAY_ENTERPRISE_MODE requires AETHERGATEWAY_DATABASE_TLS_MODE=verify-full".to_owned(),
        ));
    }
    let mode = configured
        .map(parse_ssl_mode)
...
fn parse_ssl_mode(value: &str) -> Result<PgSslMode, AppError> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(AppError::Config(format!(
            "invalid AETHERGATEWAY_DATABASE_TLS_MODE={value:?}; expected disable, allow, prefer, require, verify-ca, or verify-full"
        ))),
    }
}
...
pub(crate) fn redact_database_url(url: &str) -> String {
    ...
    let username = userinfo.split(':').next().unwrap_or("aethergateway");
...
mod tests {
    ...
    fn connection_options_apply_the_enterprise_tls_policy() {
        let url = "postgres://aethergateway:secret@db.example:5432/aethergateway";
        assert!(matches!(
    ...
    fn database_url_redaction_preserves_target_but_not_password() {
        let redacted = redact_database_url("postgres://alice:secret@db:5432/aethergateway");
        assert_eq!(redacted, "postgres://alice:<redacted>@db:5432/aethergateway");
        assert!(!redacted.contains("secret"));
    }

    #[test]
    fn deployment_values_reject_invalid_flags_and_pool_numbers() {
        assert!(parse_flag("AETHERGATEWAY_ENTERPRISE_MODE", Some("maybe")).is_err());
        assert!(parse_flag("AETHERGATEWAY_ENTERPRISE_MODE", Some("yes")).unwrap());
        assert!(parse_optional_u32("AETHERGATEWAY_DATABASE_MAX_CONNECTIONS", Some("many")).is_err());
        assert!(parse_optional_u64("AETHERGATEWAY_DATABASE_ACQUIRE_TIMEOUT_SECS", Some("-1")).is_err());
    }
    ...
    fn database_url_validation_never_echoes_a_secret() {
        let error = validate_database_url(
            ...
            "not-postgres://alice:super-secret@db/aethergateway",
        )
        .unwrap_err()
        .to_string();
        assert!(error.contains("AETHERGATEWAY_DATABASE_URL"));
    ...
    fn enterprise_deployment_requires_control_database_and_strict_tls() {
        ...
        .unwrap_err()
        .to_string();
        assert!(missing_database.contains("AETHERGATEWAY_DATABASE_URL"));

        let weak_tls = validate_values(DatabaseValidationValues {
            enterprise_mode: Some("1"),
            control_url: Some("postgres://aethergateway:secret@db:5432/aethergateway"),
