...
ModelDock already exposes health, metrics, request evidence, budgets, Provider
...
- ModelDock remains the policy and data boundary. The agent has no PostgreSQL
...
- A bounded SQLite spool stores observations awaiting delivery. PostgreSQL in
  ModelDock is the authoritative incident ledger.
