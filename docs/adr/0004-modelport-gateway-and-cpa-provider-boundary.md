# ADR-0004: AetherGateway Gateway And CPA Provider Boundary
...
CLIProxyAPI, commonly abbreviated CPA. AetherGateway already owns the client API,
authentication, model resolution, smart routing, policy, quota, fallback,
health, accounting, and operational evidence.

Putting CPA or LiteLLM in front of AetherGateway, or treating several gateways as
...
AetherGateway remains the only client-facing gateway and the source of truth for
policy, routing, retry/fallback, quota, health, and request evidence.

CPA is optional and appears only as two internal AetherGateway Providers:
...
AetherGateway stores only CPA's client key. CPA owns OAuth tokens, auth files,
account cooldown, and bounded credential selection. CPA's management API is
not a AetherGateway Provider endpoint and is not integrated into the AetherGateway
control plane.
...
AetherGateway owns request-level retry and cross-Provider fallback. CPA starts with
request retries disabled and credential retries bounded so a single recorded
AetherGateway attempt cannot silently fan out without a reviewed operational
limit.
...
evaluated independently and reimplemented only when they fit AetherGateway's typed
and auditable contracts.

## Consequences

- Applications keep one AetherGateway endpoint and one scoped client credential.
- Provider health and request evidence distinguish CPA Codex from CPA Claude.
- CPA can evolve or be removed without changing the public AetherGateway API.
- CPA account-level attempts are not currently first-class AetherGateway ledger
...
- CPA in front of AetherGateway: bypasses AetherGateway's client governance and makes
  AetherGateway an upstream implementation detail.
- AetherGateway and CPA as equal public gateways: creates two policy, credential,
...
- Unbounded CPA credential traversal: makes one AetherGateway attempt have
