...
AetherGateway is a single-process Rust gateway with a separate React dashboard. Its
...
React dashboard -> local password or OIDC Authorization Code + PKCE
                -> /admin/* AetherGateway cookie-session control plane
...
The diagram above is the implemented v0.1.x data plane. AetherGateway is evolving
...
The target keeps inference engines outside AetherGateway:

```text
Client/Harness
      |
      v
AetherGateway protocol, policy, routing, ledger, and control APIs
      |
      +---------------- Hosted Provider API
      |
...
AetherGateway owns desired state, observed inventory, governance, reconciliation,
and durable evidence. An external runtime owns device-specific execution and
runtime-native caches. The Dashboard calls AetherGateway control APIs; it never
...
| --- | --- |
| Client/Harness | Calls a supported AetherGateway protocol edge. Claude Code, Codex CLI, Qwen Code, SDKs, and internal applications are clients, not Provider types. DeepSeek remains a Provider/model family unless a distinct caller contract is introduced and verified. |
...
lifecycle, and locally observed capacity. AetherGateway remains useful without a
managed GPU.
...
```text
clients -> AetherGateway -> cpa_codex  -> CPA -> Codex OAuth accounts
                    -> cpa_claude -> CPA -> Claude OAuth accounts
                    -> other hosted/local Providers
```

AetherGateway remains the only public client endpoint and owns authentication,
...
7. AetherGateway atomically creates the tenant-scoped request row. When an
...
8. For each attempt, AetherGateway selects a provider credential, checks API-key
...
pair to a local AetherGateway user and produces the same HttpOnly console session;
automatic provisioning, when enabled, creates only an ordinary `user`. The
identity-provider token and the AetherGateway session cookie are never accepted as
...
AetherGateway now establishes the upstream connection and checks its initial HTTP
...
must reach `[DONE]` or a `finish_reason`, after which AetherGateway emits
...
`buffer_stream_text=true` is a distinct compatibility path. AetherGateway sends a
...
- Optional OIDC uses Authorization Code flow with PKCE, state, nonce, and a
  ...
  Verified identities resolve to local users and receive the normal AetherGateway
...
- Forwarded client IP headers are accepted only from configured trusted peers.
  AetherGateway walks `X-Forwarded-For` from the connected peer right-to-left,
...
- Treating one local runtime repository, engine, or Harness as the AetherGateway
  product model.
