...
This guide covers a running AetherGateway instance. Docker-specific storage and
...
that returned text instead. These are request-level observations; AetherGateway
...
The row's `statusCode` is AetherGateway's effective client-facing HTTP status for
...
when AetherGateway receives a trusted Provider-reported amount or applies an exact,
...
adapter exposed Provider usage, while `local-estimate` means AetherGateway used its
...
A request is chargeable only after AetherGateway actually starts an upstream
...
explicit preview/apply operation; AetherGateway does not silently evict rows from
an in-document ring.
...
AetherGateway never intentionally persists prompt, response, tool name, tool
...
logging integration, crash dump, or host outside AetherGateway's ledger.
...
console session, `X-AetherGateway-CSRF: 1`, and the normal same-origin check.
...
zero-residue test, use an isolated AetherGateway database, export the result, and
...
AetherGateway targets single-host personal and small-team traffic. Upstream
...
If no permit is immediately available, AetherGateway returns HTTP 429
