...
AetherGateway stores all runtime state in PostgreSQL. The Compose stack supplies it;
you do not need to install PostgreSQL on the host.

## 2. Create Local Configuration

```bash
git clone --depth 1 https://github.com/PeaceMaker-best/AetherGateway.git
cd AetherGateway
scripts/setup.sh
```
...
[GitHub Releases](https://github.com/PeaceMaker-best/AetherGateway/releases). Source builds
...
AetherGateway fails closed for cloud egress when a project has no policy. Before
