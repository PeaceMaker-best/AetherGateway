...
AetherGateway accepts Anthropic Tool Use and the scoped OpenAI Chat Completions
...
be replayed. AetherGateway therefore sends `thinking.type=disabled` only on this
...
the OpenAI tool-role content with a stable `AetherGateway tool execution error`
...
`parallel_tool_calls=false`, AetherGateway also injects the upstream parameter when
...
For `/v1/messages`, AetherGateway emits Anthropic-style events:
...
AetherGateway emits the terminal form expected by the originating client protocol.
...
Messages responses rejected by the declared JSON Schema. AetherGateway schedules
