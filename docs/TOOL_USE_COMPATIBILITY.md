...
ModelDock accepts Anthropic Tool Use and the scoped OpenAI Chat Completions
...
be replayed. ModelDock therefore sends `thinking.type=disabled` only on this
...
the OpenAI tool-role content with a stable `ModelDock tool execution error`
...
`parallel_tool_calls=false`, ModelDock also injects the upstream parameter when
...
For `/v1/messages`, ModelDock emits Anthropic-style events:
...
ModelDock emits the terminal form expected by the originating client protocol.
...
Messages responses rejected by the declared JSON Schema. ModelDock schedules
