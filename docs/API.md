...
AetherGateway exposes Anthropic Messages and a scoped OpenAI Chat Completions data
...
| `GET /admin/auth/oidc/callback` | none | Requires the matching browser-flow cookie, validates and consumes the OIDC callback, resolves the local user, issues the AetherGateway console cookie, and redirects to a local return path. |
...
The response is `{"input_tokens":N}`. AetherGateway validates the same Anthropic
...
upstream tokenizer and chat template; AetherGateway does not use its characters/4
...
`store=false` or `null`, and `n=1`. AetherGateway removes `store` before Provider
...
selected Provider is OpenAI-compatible; AetherGateway validates and forwards it so
...
supplied, AetherGateway uses 4096 only for local estimation and for an Anthropic
...
`stream_options.include_usage=true`, AetherGateway preserves an OpenAI-compatible
...
OpenAI-compatible stream must provide `[DONE]` or a `finish_reason`; AetherGateway
...
For an OpenAI-compatible provider with `buffer_stream_text=true`, AetherGateway
...
AetherGateway accepts `x-request-id`; otherwise middleware assigns one. The response
...
trace; AetherGateway does not attach trace/span-parent semantics.
...
Provider attempt. AetherGateway stores only SHA-256 of the key plus a protocol/body
...
`/v1/chat/completions`; clients use a AetherGateway API key (or the explicitly
enabled legacy router token), while upstream Provider credentials remain on
the AetherGateway server. See [OIDC Console Sign-In](OIDC.md).
...
- `POST /admin/retention/run`: administrator-only metadata-retention preview or
  apply. It requires the console session, `X-AetherGateway-CSRF: 1`, and normal
...
Data-plane `X-AetherGateway-Organization-Id`, `X-AetherGateway-Project-Id`, and
`X-AetherGateway-Environment-Id` headers are optional assertions: a partial tuple is
...
as `X-AetherGateway-Change-Request-Id` when dual approval is required or voluntarily
...
AetherGateway may display and alert on the balance, but recharge, refunds, invoices,
...
and requires an admin session plus `X-AetherGateway-CSRF`. Likewise,
...
means AetherGateway used its request-based estimate. Both remain operational cost
estimates because AetherGateway applies its local pricing table. When an
...
model emitted a new call, and AetherGateway still does not retain tool names,
