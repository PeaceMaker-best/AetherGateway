...
Dashboard 是 AetherGateway 的管理界面，不是聊天应用。AetherGateway 负责鉴权、路由、
策略、协议转换和证据记录，不负责运行模型。

如果要复现可选的本机 Qwen 参考路径，请阅读
[本地 Qwen 参考适配](LOCAL_INFERENCE_STACK.md)。它把静态契约检查、外部 GPU
Runtime 和 AetherGateway 分成可独立验证的阶段；AetherGateway 不依赖该集成或其仓库。
...
| 密钥 | 给谁使用 | 是否可以交给客户端 |
| --- | --- | --- |
| Provider Key | AetherGateway 访问上游 Provider | 不可以 |
| AetherGateway API Key | Claude Code、SDK 或内部应用访问 AetherGateway | 可以，但应限制作用域 |
...
- 客户端只知道 AetherGateway 地址和 AetherGateway Key；
...
| Gateway request | 客户端向 AetherGateway 发出的一次请求 |
| Provider attempt | AetherGateway 对某个 Provider 的一次实际尝试；回退时可能有多个 |
...
4. 返回 401/403：检查使用的是 AetherGateway Key、账号状态和策略。
