...
fn cli_deployment_preflight_enforces_the_enterprise_security_profile() {
    ...
    assert!(valid_text.contains("AetherGateway configuration valid"));
}
...
fn cli_deployment_preflight_accepts_a_valid_local_environment() {
    ...
    assert!(text.contains("AetherGateway configuration valid"));
}
...
fn openai_upstream_base_url_cannot_point_back_to_modelport() {
    ...
    assert!(unspecified_host_text.contains("points back to this AetherGateway listener"));
}
