...
async fn current_schema_migration_preserves_legacy_requests_and_attempts() {
    ...
    let mut connection = PgConnection::connect(&database_url)
        .await
        .expect("connect to AETHERGATEWAY_TEST_DATABASE_URL");
    let schema = format!("aethergateway_migration_{}", Uuid::new_v4().simple());
    connection
    ...
    let migration_result = async {
        ...
        connection
            .execute(
                "CREATE TABLE aethergateway_state (
        ...
        connection
            .execute(
                "INSERT INTO aethergateway_state (namespace, document)
        ...
        .await?;
        let attempt_count =
            sqlx::query_scalar::<_, i64>("SELECT count(*) FROM aethergateway_provider_attempts")
                .fetch_one(&mut connection)
                .await?;
        ...
        let routing_table_exists = sqlx::query_scalar::<_, bool>(
            "SELECT to_regclass('aethergateway_routing_decisions') IS NOT NULL",
        )
        .fetch_one(&mut connection)
        .await?;
        let legacy_state = sqlx::query_as::<_, (serde_json::Value, i64)>(
            "SELECT document, revision
             FROM aethergateway_state
...
async fn embedded_migrations_preserve_restored_operational_row_counts() {
    let Ok(database_url) = std::env::var("AETHERGATEWAY_MIGRATION_DRILL_DATABASE_URL") else {
        return;
    };
    ...
    let requests_before =
        sqlx::query_scalar::<_, i64>("SELECT count(*) FROM aethergateway_gateway_requests")
            .fetch_one(&pool)
            .await
        ...
        "SELECT count(*)
         ...
         WHERE request_id LIKE 'retained:%'
           AND request_fingerprint <> encode(
               sha256(
                   convert_to(
                       'aethergateway-retained-request-fingerprint-v1:' || ledger_id,
                       'UTF8'
                   )
               ),
               'hex'
