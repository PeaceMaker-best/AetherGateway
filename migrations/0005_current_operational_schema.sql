...
UPDATE aethergateway_gateway_requests AS request
...
ALTER TABLE aethergateway_gateway_requests
    ...
    ADD CONSTRAINT aethergateway_gateway_requests_path_check
        CHECK (request_path IN ('/v1/messages', '/v1/chat/completions')),
    ADD CONSTRAINT aethergateway_gateway_requests_traffic_class_check
        CHECK (traffic_class IN ('business', 'synthetic', 'diagnostic')),
    ADD CONSTRAINT aethergateway_gateway_requests_latency_check
        CHECK (
            latency_ms >= 0
            AND (first_byte_latency_ms IS NULL OR first_byte_latency_ms >= 0)
            AND retry_count >= 0
        ),
    ADD CONSTRAINT aethergateway_gateway_requests_provider_snapshot_check
        CHECK (
            (provider_id IS NULL
    ...
    ADD CONSTRAINT aethergateway_gateway_requests_tool_outcome_check
        CHECK (
            tool_outcome IN (
                'not_requested',
...
UPDATE aethergateway_provider_attempts AS attempt
...
ALTER TABLE aethergateway_provider_attempts
    ADD CONSTRAINT aethergateway_provider_attempts_latency_check
        CHECK (
            latency_ms >= 0
            AND (first_byte_latency_ms IS NULL OR first_byte_latency_ms >= 0)
            AND retry_count >= 0
        ),
    ADD CONSTRAINT aethergateway_provider_attempts_tool_outcome_check
        CHECK (
            tool_outcome IN (
                'not_requested',
                'continuation_tool_called',
...
CREATE INDEX aethergateway_gateway_requests_operational_created_idx
...
CREATE INDEX aethergateway_gateway_requests_api_key_created_idx
    ON aethergateway_gateway_requests (api_key_id, created_at DESC)
    WHERE api_key_id IS NOT NULL;

CREATE INDEX aethergateway_gateway_requests_team_created_idx
    ON aethergateway_gateway_requests (team_id, created_at DESC)
    WHERE team_id IS NOT NULL;

CREATE INDEX aethergateway_gateway_requests_principal_created_idx
    ON aethergateway_gateway_requests (principal_id, created_at DESC);

CREATE INDEX aethergateway_gateway_requests_provider_created_idx
    ON aethergateway_gateway_requests (provider_id, created_at DESC)
    WHERE provider_id IS NOT NULL;

CREATE TABLE aethergateway_audit_events (
    ...
    CONSTRAINT aethergateway_audit_events_severity_check
        CHECK (severity IN ('info', 'warning', 'error')),
    CONSTRAINT aethergateway_audit_events_text_check
        CHECK (
...
CREATE INDEX aethergateway_audit_events_occurred_idx
    ON aethergateway_audit_events (occurred_at DESC, event_id DESC);
