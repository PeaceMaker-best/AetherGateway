...
CREATE TABLE aethergateway_usage_reservations (
    ...
    CONSTRAINT aethergateway_usage_reservations_request_unique UNIQUE (
        organization_id,
        project_id,
        environment_id,
        request_ledger_id
    ),
    CONSTRAINT aethergateway_usage_reservations_state_check
        CHECK (state IN ('reserved', 'settled', 'released')),
    CONSTRAINT aethergateway_usage_reservations_amounts_check CHECK (
...
CREATE INDEX aethergateway_usage_reservations_subject_open_idx
    ON aethergateway_usage_reservations (quota_subject_id, created_at)
    WHERE state = 'reserved' AND quota_subject_id IS NOT NULL;

CREATE INDEX aethergateway_usage_reservations_team_open_idx
    ON aethergateway_usage_reservations (team_id, created_at)
    WHERE state = 'reserved' AND team_id IS NOT NULL;

CREATE INDEX aethergateway_usage_reservations_user_open_idx
    ON aethergateway_usage_reservations (user_id, created_at)
