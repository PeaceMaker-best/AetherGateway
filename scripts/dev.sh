...
Local source development (PostgreSQL must already be available):
  ...
  stop                Stop only native AetherGateway processes from this checkout.
...
start_gateway() {
  ...
  if owned_pid "$pid" || [[ -n "$(project_pids)" ]]; then
    if health_ok; then
      log "AetherGateway is already running at $(base_url)"
      return
    fi
  ...
  if wait_for_health 30 1; then
    log "AetherGateway started, pid $pid"
    status_gateway
  else
    log "AetherGateway failed to become healthy"
...
stop_gateway() {
  ...
  log "stopping AetherGateway pids: ${pids[*]}"
  for pid in "${pids[@]}"; do
    if owned_pid "$pid"; then
...
case "$command_name" in
  run)
    load_env
    setup_cc_fallback
    log "starting AetherGateway in foreground at $(base_url)"
