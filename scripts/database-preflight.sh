...
die() {
  printf '[aethergateway-database] ERROR: %s\n' "$*" >&2
  exit 1
}
...
main() {
  ...
  state_rows="$(docker compose -f "$COMPOSE_FILE" exec -T postgres sh -c \
    'exec psql --username="$POSTGRES_USER" --dbname="$POSTGRES_DB" --tuples-only --no-align --command="select count(*) from aethergateway_state"')"

  printf '[aethergateway-database] running image=%s server_major=%s volume=%s destination=%s\n' \
    "$running_image" "$running_major" "$volume_name" "$volume_destination"
  printf '[aethergateway-database] declared image=%s server_major=%s volume=%s\n' \
  ...
  printf '[aethergateway-database] alignment and migration preflight passed\n'
}
