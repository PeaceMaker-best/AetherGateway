...
check_linux_platform() {
  ...
  if [[ "$kernel" == "Linux" ]]; then
    ...
    fail "AetherGateway development and maintained scripts require Linux; detected ${kernel:-unknown}"
  fi
...
check_provider_env() {
  ...
  if [[ "${ANTHROPIC_BASE_URL:-}" == "$(base_url)" ]]; then
    ok "ANTHROPIC_BASE_URL points to AetherGateway"
  else
...
check_vscode_settings_text() {
  ...
  if grep -Fq '"ANTHROPIC_BASE_URL"' "$settings_file" && grep -Fq "$(base_url)" "$settings_file"; then
    ok "VS Code settings points ANTHROPIC_BASE_URL to AetherGateway"
  else
...
if [[ "$failures" -gt 0 ]]; then
  printf '\nAetherGateway doctor (%s) failed: %d failure(s), %d warning(s).\n' \
    "$mode" "$failures" "$warnings" >&2
  exit 1
fi

printf '\nAetherGateway doctor (%s) passed: %d warning(s).\n' "$mode" "$warnings"
